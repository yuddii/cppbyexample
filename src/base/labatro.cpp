...
struct card
{
    ...
    int getRank()
    {
        return enhance == "Stone" ? 100 : number == 1 ? 14
                                                      : number;
    }
    int getNumber()
    {
    ...
    {
        int tmp1 = x.enhance == "Stone" ? 100 : x.number == 1 ? 14
                                                              : x.number;
        int tmp2 = y.enhance == "Stone" ? 100 : y.number == 1 ? 14
                                                              : y.number;
...
struct Deck
{
    ...
    int GetType()
    {
        ...
        if (cnt < 5)
            isSameSuit = isStraight = 0;
        else
        {
            int Suitlast = 1;
            while (c[Suitlast].getSuit() == 5 && Suitlast <= cnt)
                ++Suitlast;
            if (Suitlast < cnt)
            {
                for (int i = Suitlast + 1; i <= cnt; i++)
                    if (c[i].getSuit() != 5 && c[i].getSuit() != c[Suitlast].getSuit())
                        isSameSuit = 0;
            }
            for (int i = 2; i <= 5; i++)
            {
                if (Shortcut && c[i].getRank() != c[i - 1].getRank() + 1 && c[i].getRank() != c[i - 1].getRank() + 2)
                    isStraight = 0;
                if (!Shortcut && c[i].getRank() != c[i - 1].getRank() + 1)
                    isStraight = 0;
            }
        }
        int tmp[6], qn = 0;
        ...
        for (int i = 1; i <= cnt; i++)
        {
            if (i == 1 || c[i].getNumber() != c[i - 1].getNumber())
                ++qn;
            ++tmp[qn];
        }
        if (tmp[1] == 5 && isSameSuit)
            return 1;
        if (cnt == 5 && qn == 2 && (tmp[1] == 3 || tmp[2] == 3) && isSameSuit)
            return 2;
        if (tmp[1] == 5)
            return 3;
        ...
        if (tmp[1] == 4 || tmp[2] == 4)
            return 5;
        if (cnt == 5 && qn == 2 && (tmp[1] == 3 || tmp[2] == 3))
            return 6;
        if (isSameSuit)
            return 7;
        if (isStraight)
            return 8;
        if (tmp[1] == 3 || tmp[2] == 3 || tmp[3] == 3)
            return 9;
        if ((tmp[1] == 2 && (tmp[2] == 2 || tmp[3] == 2)) || (tmp[2] == 2 && tmp[3] == 2))
            return 10;
        if (tmp[1] == 2 || tmp[2] == 2 || tmp[3] == 2 || tmp[4] == 2)
            return 11;
        return 12;
    }
...
void useTarot(const consumables &tmp)
{
    ...
    if (tmp.name == "The Magician[I]")
        SelecttoEnhance(2, "Lucky");
    if (tmp.name == "The Empress[III]")
        SelecttoEnhance(2, "Mult");
    if (tmp.name == "The Hierophant[V]")
        SelecttoEnhance(2, "Bonus");
    if (tmp.name == "The Lovers[VI]")
        SelecttoEnhance(1, "Wild");
    if (tmp.name == "The Chariot[VII]")
        SelecttoEnhance(1, "Steel");
    if (tmp.name == "Justice[VIII]")
        SelecttoEnhance(1, "Glass");
    if (tmp.name == "The Devil[XV]")
        SelecttoEnhance(1, "Gold");
    if (tmp.name == "The Tower[XVI]")
        SelecttoEnhance(1, "Stone");

    if (tmp.name == "The Hermit[IX]")
        p1.money += min(p1.money, 20);
    if (tmp.name == "The Wheel of Fortune[X]")
    {
        if (rand() % 4 == 0 && p1.jcnt)
        {
            int t = rand() % 3;
            p1.j[rand() % p1.jcnt + 1].edition = (t == 0 ? "Polychrome" : t == 1 ? "Holographic"
                                                                                 : "Foil");
        }
        else
        {
    ...
    if (tmp.name == "The Star[XVII]")
        SelecttoSuit(3, 1);
    if (tmp.name == "The Moon[XVIII]")
        SelecttoSuit(3, 2);
    if (tmp.name == "The Sun[XIX]")
        SelecttoSuit(3, 3);
    if (tmp.name == "The World[XXI]")
        SelecttoSuit(3, 4);
...
void useSpectral(const consumables &tmp)
{
    ...
    if (tmp.name == "Trance")
        SelecttoSeal(1, "Blue");
    if (tmp.name == "Meidum")
        SelecttoSeal(1, "Purple");

    if (tmp.name == "Black Hole")
        for (int i = 1; i <= 12; i++)
            PHupgrade(i);
...
void useConsumables(const consumables &tmp)
{
    ...
    if (tmp.type == "planet")
        usePlanet(tmp);
    else if (tmp.type == "tarot")
        useTarot(tmp);
    else if (tmp.type == "spectral")
        useSpectral(tmp);
...
struct SHOP
{
    ...
    void rerollC()
    {
        ...
        while (!tmp)
        {
            ...
            for (int i = 1; i <= p1.jcnt; i++)
                if (c2.type == 1 && c2.j == p1.j[i])
                    tmp = 0;
            if (c2.type == 1 && c1.type == 1 && c2.j == c1.j)
                tmp = 0;
        }
    }
    ...
    void openBuffoonPack(pack tmp)
    {
        ...
        if (p1.JcntMax + tmp.total > 10)
            cout << "Sorry there are no more Jokers\n";
    ...
    void buyPack(pack tmp, int num)
    {
        ...
        else
        {
            ...
            if (tmp.name == "Standard Pack")
            {
                openStandardPack(tmp);
            }
            else if (tmp.name == "Buffoon Pack")
            {
                openBuffoonPack(tmp);
            }
            else if (tmp.name == "Celestial Pack")
            {
                openCelestialPack(tmp);
            }
            else if (tmp.name == "Arcana Pack")
            {
                openArcanaPack(tmp);
            }
            else if (tmp.name == "Spectral Pack")
            {
                openSpectralPack(tmp);
            }
...
struct challenge
{
    ...
    void reTrigger(card tmp)
    {
        ...
        tmp.show();
        chips += tmp.getNumber() == 1 ? 11 : tmp.getNumber() > 10 ? 10
                                                                  : tmp.getNumber();
        cout << "+" << (tmp.getNumber() == 1 ? 11 : tmp.getNumber() > 10 ? 10
                                                                         : tmp.getNumber())
             << " Chip\n";
        showChipandMult();
